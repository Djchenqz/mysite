...
def send_notification(sender, instance, **kwargs):
	...
	if instance.reply_to is None:
		...
		if instance.content_type.model == 'blog':
			...
			verb = '{0} 评论了你的博客《{1}》'.format(instance.user.get_username_or_nickname(), blog.title)
		else:
			raise Exception('unknow cpmment object type')
	else:
		# 回复
		recipient = instance.reply_to
		verb = '{0} 回复了你的评论"{1}"'.format(instance.user.get_username_or_nickname(), strip_tags(instance.parent.text))
